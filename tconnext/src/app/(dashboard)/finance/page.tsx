...
export default function FinancePage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const { bookings: bookingsData, programs: programsData, agents: agentsData, invoices: invoicesData } = await response.json()

      // Also fetch invoice items using client
      ...
      const bookingsWithRevenue: FinanceBooking[] = (bookingsData || []).map((booking: any) => {
        const program = booking.program as { base_price?: number } | undefined
        const basePrice = program?.base_price || 0
        const calculated_revenue = basePrice * ((booking.adults || 0) + (booking.children || 0) * 0.5)
  ...
  useEffect(() => {
    ...
    // Uninvoiced bookings - amount and count
    const uninvoicedBookingsList = bookings.filter(b => !b.is_invoiced)
    const amountToInvoice = uninvoicedBookingsList.reduce((sum, b) => sum + b.calculated_revenue, 0)
    const bookingsToInvoice = uninvoicedBookingsList.length

    // Invoiced but unpaid bookings
    const invoicedUnpaidBookings = bookings.filter(b => b.is_invoiced && b.invoice_status === 'sent')
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      // Only select uninvoiced bookings
      const uninvoiced = filteredBookings.filter(b => !b.is_invoiced).map(b => b.id)
      setSelectedBookings(new Set(uninvoiced))
    } else {
  ...
  const selectedBookingsByAgent = useMemo(() => {
    const selected = filteredBookings.filter(b => selectedBookings.has(b.id) && !b.is_invoiced)
  ...
  const uninvoicedSelectedCount = useMemo(() => {
    return filteredBookings.filter(b => selectedBookings.has(b.id) && !b.is_invoiced).length
  }, [filteredBookings, selectedBookings])
    ...
    <div className="space-y-6">
      ...
      >
        <Button
          ...
          disabled={uninvoicedSelectedCount === 0}
        >
          <Receipt className="w-4 h-4 mr-2" />
          Create Invoice ({uninvoicedSelectedCount})
        </Button>
      </PageHeader>
              ...
              <TableHeader>
                <TableRow>
                  <TableHead className="w-12">
                    <Checkbox
                      checked={selectedBookings.size > 0 && selectedBookings.size === filteredBookings.filter(b => !b.is_invoiced).length}
