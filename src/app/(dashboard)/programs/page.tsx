...
export default function ProgramsPage() {
  ...
  const startEdit = (program: Program) => {
    ...
    setFormData({
      ...
      itinerary_title: (program as any).itinerary_title || 'Itinerary',
    })
      ...
      if (editingId) {
        ...
        const { error } = await supabase
          ...
          .update({
            ...
            itinerary_title: formData.itinerary_title || 'Itinerary',
          })
          .eq('id', editingId)
  ...
  // Handle Direct Booking toggle - auto-save when enabled for new programs
  const handleDirectBookingToggle = async (checked: boolean) => {
    setFormData({ ...formData, direct_booking_enabled: checked })
    
    // If enabling and this is a new program (isCreating), save it first
    if (checked && isCreating && formData.name) {
      setSaving(true)
      try {
        const supabase = createClient()
        const { data, error } = await supabase
          ...
          .insert({
            ...
            adult_selling_price: formData.adult_selling_price,
            child_selling_price: formData.child_selling_price,
            default_pickup_time: formData.default_pickup_time,
            booking_cutoff_time: formData.booking_cutoff_time,
            status: formData.status,
            come_direct_location: formData.come_direct_name ? {
              name: formData.come_direct_name,
              address: formData.come_direct_address,
              google_maps_url: formData.come_direct_google_maps_url,
              contact_info: formData.come_direct_contact_info,
            } : null,
            slug: formData.slug || null,
            thumbnail_url: formData.thumbnail_url || null,
        ...
        toast.success('Program saved! You can now upload images.')
        setIsCreating(false)
        setEditingId(data.id)
        await fetchPrograms()
      } catch (error: any) {
        toast.error(error.message || 'Failed to save program')
        setFormData({ ...formData, direct_booking_enabled: false })
      } finally {
        setSaving(false)
      }
    }
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Short nickname for admin"
            />
          </div>
        </div>
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <Switch
              ...
              onCheckedChange={handleDirectBookingToggle}
              disabled={saving}
            />
          </div>
          <p className="text-xs text-muted-foreground">
            Enable this program on your public booking page for customers to book and pay online.
            {isCreating && formData.name && ' (Enabling will auto-save the program)'}
          </p>
          ...
          {formData.direct_booking_enabled && (
            <div className="space-y-3 pl-6 border-l-2 border-green-200">
              ...
              <div className="space-y-3 pt-4 border-t">
                <div className="space-y-2">
                  <Label htmlFor="itinerary_title" className="text-xs font-medium">Accordion Title</Label>
                  <Input
                    id="itinerary_title"
                    value={formData.itinerary_title}
                    onChange={(e) => setFormData({ ...formData, itinerary_title: e.target.value })}
                    placeholder="Itinerary"
                    className="h-8"
                  />
                  <p className="text-xs text-muted-foreground">
                    Title shown on the accordion header (e.g., "Itinerary", "Day Schedule", "Program Details")
                  </p>
                </div>
                
                <div className="space-y-2">
                  <Label htmlFor="itinerary_html" className="text-xs font-medium">Content (HTML/Rich Text)</Label>
                  <p className="text-xs text-muted-foreground">
  ...
  // Full-width edit view
  const renderFullWidthEditView = (isNew = false) => {
    const currentProgram = programs.find(p => p.id === editingId)
    const programColor = isNew ? formData.color : (currentProgram?.color || formData.color || '#3B82F6')
      ...
      <div className="space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            {/* Program Switcher - only show when editing existing */}
            {!isNew && programs.length > 1 && (
              <Select
                value={editingId || ''}
                onValueChange={(value) => {
                  const program = programs.find(p => p.id === value)
                  if (program) startEdit(program)
                }}
              >
                <SelectTrigger className="w-[240px] h-9">
                  <SelectValue placeholder="Switch program..." />
                </SelectTrigger>
                <SelectContent>
        ...
        <Card className="border" style={{ borderLeftWidth: '4px', borderLeftColor: programColor }}>
          <CardHeader className="py-3 px-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <CardTitle className="text-lg">
                  {isNew ? 'Create New Program' : (formData.nickname || formData.name || 'Edit Program')}
                </CardTitle>
                {!isNew && formData.status && (
                  <Badge variant={statusColors[formData.status as EntityStatus]} className="text-xs">
                    {formData.status}
                  </Badge>
                )}
              </div>
          ...
          <CardContent className="pt-4 space-y-5 px-4">
            {/* Basic Info Section */}
            <div className="space-y-3">
              <h3 className="font-medium text-xs text-muted-foreground uppercase tracking-wide border-b pb-2">Basic Information</h3>
              <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
                <div className="space-y-1 col-span-2 lg:col-span-1">
                  <Label htmlFor="name" className="text-xs">Program Name *</Label>
                  <Input
                    id="name"
                    value={formData.name}
                    onChange={(e) => setFormData({ ...formData, name: e.target.value })}
            ...
            <div className="grid gap-4 lg:grid-cols-2">
              {/* Color Picker */}
              <div className="space-y-3">
                <h3 className="font-medium text-xs text-muted-foreground uppercase tracking-wide border-b pb-2">Program Color</h3>
                <div className="flex items-center gap-3">
              ...
              {/* Pricing */}
              <div className="space-y-3">
                <h3 className="font-medium text-xs text-muted-foreground uppercase tracking-wide border-b pb-2">Pricing</h3>
                <div className="flex items-center gap-4 mb-2">
                  <label className="flex items-center gap-1.5 cursor-pointer text-sm">
                    <input
                      type="radio"
                      name="pricing_type_full"
                      checked={formData.pricing_type === 'single'}
                      onChange={() => setFormData({ ...formData, pricing_type: 'single' })}
            ...
            {/* Time Settings */}
            <div className="space-y-3">
              <h3 className="font-medium text-xs text-muted-foreground uppercase tracking-wide border-b pb-2">Time Settings</h3>
              <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
                <div className="space-y-1">
                  <Label className="text-xs">Default Pickup Time</Label>
                  <TimeInput
                    value={formData.default_pickup_time}
                    onChange={(value) => setFormData({ ...formData, default_pickup_time: value })}
                  />
                ...
                <div className="space-y-1">
                  <Label className="text-xs">Booking Cutoff Time</Label>
                  <TimeInput
                    value={formData.booking_cutoff_time}
                    onChange={(value) => setFormData({ ...formData, booking_cutoff_time: value })}
                  />
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 border-b pb-2">
                ...
                <h3 className="font-medium text-xs text-muted-foreground uppercase tracking-wide">Come Direct Location</h3>
                <span className="text-xs text-muted-foreground">— Meeting point for self-arrival customers</span>
              </div>
              <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
                <div className="space-y-1">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between border-b pb-2">
                <div className="flex items-center gap-2">
                  ...
                  <h3 className="font-medium text-xs text-muted-foreground uppercase tracking-wide">Direct Online Booking</h3>
                </div>
                <div className="flex items-center gap-2">
                  <Switch
                    checked={formData.direct_booking_enabled}
                    ...
                    disabled={saving}
                  />
                  <span className="text-xs font-medium">{formData.direct_booking_enabled ? 'Enabled' : 'Disabled'}</span>
                </div>
              </div>

              {formData.direct_booking_enabled && (
                <div className="space-y-3 pl-3 border-l-2 border-green-300">
                  {/* URL Slug & Thumbnail Row */}
                  <div className="grid gap-3 grid-cols-2 lg:grid-cols-4">
                    <div className="space-y-1">
                    ...
                    <div className="space-y-1">
                      ...
                      <div className="flex items-center gap-2">
                        {formData.thumbnail_url ? (
                          <>
                            <div className="w-12 h-9 rounded overflow-hidden border bg-muted flex-shrink-0">
                              <img src={formData.thumbnail_url} alt="Thumb" className="w-full h-full object-cover" />
                            </div>
                            <Button variant="outline" size="sm" className="h-9 text-xs" onClick={() => thumbnailInputRef.current?.click()} disabled={uploadingThumbnail}>
                              Change
                            </Button>
                            <Button variant="ghost" size="sm" className="h-9 text-xs text-destructive" onClick={handleThumbnailDelete} disabled={uploadingThumbnail}>
                              <X className="h-3 w-3" />
                            </Button>
                          </>
                        ) : (
                          <Button variant="outline" size="sm" className="h-9 text-xs" onClick={() => thumbnailInputRef.current?.click()} disabled={uploadingThumbnail || !editingId}>
                            {uploadingThumbnail ? <><Spinner size="sm" className="mr-1" /> Uploading</> : <><Upload className="h-3 w-3 mr-1" /> Upload</>}
                          </Button>
                        )}
                  ...
                  {/* Short Description */}
                  <div className="space-y-1">
                    <Label className="text-xs">Short Description (for booking page)</Label>
                    <Textarea
                      value={formData.short_description}
                      onChange={(e) => setFormData({ ...formData, short_description: e.target.value })}
                  ...
                  {/* Brochure & Itinerary Row */}
                  <div className="grid gap-4 lg:grid-cols-2">
                    {/* Brochure Images */}
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        <Label className="text-xs">Brochure Images (up to 10)</Label>
                        {editingId && formData.brochure_images.length < 10 && (
                          <>
                            <input ref={brochureInputRef} type="file" accept="image/*" onChange={handleBrochureUpload} className="hidden" />
                            <Button variant="outline" size="sm" className="h-7 text-xs" onClick={() => brochureInputRef.current?.click()} disabled={uploadingBrochure}>
                              {uploadingBrochure ? <Spinner size="sm" /> : <><Upload className="h-3 w-3 mr-1" /> Add</>}
                            </Button>
                          </>
                        )}
                      </div>
                      {formData.brochure_images.length > 0 ? (
                        <div className="flex gap-1.5 flex-wrap">
                          {formData.brochure_images.map((url, index) => (
                            <div key={index} className="relative group">
                              <div className="w-14 h-14 rounded overflow-hidden border bg-muted">
                                <img src={url} alt={`Brochure ${index + 1}`} className="w-full h-full object-cover" />
                              </div>
                              <Button variant="destructive" size="icon" className="absolute -top-1 -right-1 h-5 w-5 opacity-0 group-hover:opacity-100" onClick={() => handleBrochureDelete(url)} disabled={uploadingBrochure}>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2">
                        <Label className="text-xs">Accordion Title</Label>
                        <Input
                          value={formData.itinerary_title}
                          onChange={(e) => setFormData({ ...formData, itinerary_title: e.target.value })}
                          placeholder="Itinerary"
                      ...
                      <Textarea
                        value={formData.itinerary_html}
                        onChange={(e) => setFormData({ ...formData, itinerary_html: e.target.value })}
                        placeholder="<h3>Schedule</h3><p><strong>08:00</strong> - Pick up</p>..."
                        rows={4}
  ...
  // If editing or creating, show full-width view
  if (editingId || isCreating) {
    return (
      <ProtectedPage pageKey="programs" pageName="Program Setup">
        {renderFullWidthEditView(isCreating)}
      </ProtectedPage>
    )
  }

  // Otherwise show the program list
  return (
    <ProtectedPage pageKey="programs" pageName="Program Setup">
    <div className="space-y-6">
      <PageHeader
