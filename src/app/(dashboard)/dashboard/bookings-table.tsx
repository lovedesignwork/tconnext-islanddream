...
export function BookingsTable({ bookings, onRefresh, companyName = '', companySettings }: BookingsTableProps) {
  ...
  const [emailConfirmDialog, setEmailConfirmDialog] = useState<{
    ...
    isSending: boolean
  }>({ open: false, booking: null, isSending: false })
  ...
  const openEmailConfirmDialog = (booking: BookingWithRelations) => {
    // For Come Direct bookings, we don't need pickup_time
    if (!booking.is_come_direct && !booking.pickup_time) {
      toast.error('Pickup time not set')
      return
    }
    setEmailConfirmDialog({ open: true, booking, isSending: false })
  }

  // Confirm and send email
  const handleConfirmSendEmail = async () => {
    const booking = emailConfirmDialog.booking
    if (!booking) return

    setEmailConfirmDialog(prev => ({ ...prev, isSending: true }))
    ...
    try {
      ...
      toast.success(booking.is_come_direct ? 'Meeting point email sent!' : 'Pickup time email sent!')
      setEmailConfirmDialog({ open: false, booking: null, isSending: false })
      onRefresh()
    } catch (error: any) {
      console.error('Send email error:', error)
      toast.error(error.message || 'Failed to send email')
      setEmailConfirmDialog(prev => ({ ...prev, isSending: false }))
    }
  }
  ...
  return (
    <>
                    ...
                    {booking.pickup_time ? (
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          {booking.customer_email && !isVoid && (booking.pickup_time || booking.is_come_direct) && (
                            <Button
                              variant="ghost"
                              size="icon"
                              className={`h-6 w-6 ${booking.pickup_email_sent ? 'text-green-600' : ''}`}
                              onClick={() => openEmailConfirmDialog(booking)}
                              title={booking.is_come_direct 
                                ? (booking.pickup_email_sent ? "Re-send meeting point email" : "Send meeting point email")
                                : (booking.pickup_email_sent ? "Re-send pickup time email" : "Send pickup time email")}
                            >
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        {(booking.pickup_time || booking.is_come_direct) && booking.customer_email && !isVoid && (
                          <DropdownMenuItem onClick={() => openEmailConfirmDialog(booking)}>
                            <Mail className="mr-2 h-4 w-4" />
                            {booking.is_come_direct
                              ? (booking.pickup_email_sent ? 'Re-send Meeting Point Email' : 'Send Meeting Point Email')
      ...
      <Dialog open={emailConfirmDialog.open} onOpenChange={(open) => {
        if (!open && !emailConfirmDialog.isSending) {
          setEmailConfirmDialog({ open: false, booking: null, isSending: false })
        }
      }}>
        <DialogContent className="sm:max-w-[450px]">
          <DialogHeader>
            <DialogTitle>
              {emailConfirmDialog.booking?.is_come_direct 
                ? 'Send Meeting Point Email' 
                : 'Send Pickup Time Email'}
            </DialogTitle>
            <DialogDescription>
              {emailConfirmDialog.booking?.pickup_email_sent
                ? 'This email has already been sent. Are you sure you want to send it again?'
                : emailConfirmDialog.booking?.is_come_direct
                  ? 'Send meeting point confirmation email to the customer?'
                  : 'Send pickup time confirmation email to the customer?'}
            </DialogDescription>
          </DialogHeader>
          
          {emailConfirmDialog.booking && (
            <div className="space-y-3 py-2">
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Customer:</span>
                <span className="font-medium">{emailConfirmDialog.booking.customer_name}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Email:</span>
                <span className="font-medium">{emailConfirmDialog.booking.customer_email}</span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-muted-foreground">Booking:</span>
                <span className="font-medium">{emailConfirmDialog.booking.booking_number}</span>
              </div>
              {emailConfirmDialog.booking.is_come_direct ? (
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Type:</span>
                  <span className="font-medium text-blue-600">Come Direct (Meeting Point)</span>
                </div>
              ) : (
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Pickup Time:</span>
                  <span className="font-medium">{emailConfirmDialog.booking.pickup_time ? formatPickupTime(emailConfirmDialog.booking.pickup_time) : '-'}</span>
                </div>
              )}
              {emailConfirmDialog.booking.pickup_email_sent && (
                <div className="bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800 rounded-md p-3 text-sm text-amber-700 dark:text-amber-400">
                  ⚠️ Email was already sent previously
                </div>
              )}
            </div>
          )}
          ...
          <DialogFooter>
            <Button
              variant="outline"
              onClick={() => setEmailConfirmDialog({ open: false, booking: null, isSending: false })}
              disabled={emailConfirmDialog.isSending}
            >
              Cancel
            </Button>
            <Button
            ...
            >
              {emailConfirmDialog.isSending ? (
                ...
                <>
                  ...
                  {emailConfirmDialog.booking?.pickup_email_sent ? 'Re-send Email' : 'Send Email'}
                </>
              )}
            </Button>
          </DialogFooter>
