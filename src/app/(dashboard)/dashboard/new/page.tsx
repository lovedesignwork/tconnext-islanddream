...
import { ArrowLeft, Save, Users, Baby, UserRound, Upload, X, Image as ImageIcon, CalendarIcon, MapPin } from 'lucide-react'
import { cn } from '@/lib/utils'
import type { Program, Hotel, Agent, AgentStaff, ReferenceHotel, Company } from '@/types'
...
export default function NewBookingPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      const [programsRes, allReferenceHotelsRes, customLocationsRes, agentsRes] = await Promise.all([
        supabase
          .from('programs')
          .select('*')
  ...
  // Fetch agent staff when agent is selected
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Only require agent_staff_id if there are staff members to select from
    const currentAgent = agents.find(a => a.id === formData.agent_id)
    if (currentAgent?.agent_type === 'direct' && agentStaff.length > 0 && !formData.agent_staff_id) {
      toast.error('Please select a source for this booking')
      return
    }
    ...
    try {
      ...
      // Generate booking number using the database function (with payment_type for suffix)
      let bookingNumber: string | null = null
      
      try {
        const { data: rpcResult, error: seqError } = await supabase
          .rpc('generate_booking_number', { 
            p_company_id: user.company_id,
            p_payment_type: formData.payment_type 
          })
        
        if (!seqError && rpcResult) {
          bookingNumber = rpcResult
        } else if (seqError) {
          console.warn('RPC generate_booking_number error:', seqError.message)
        }
      } catch (rpcError) {
        console.warn('RPC generate_booking_number failed, using fallback:', rpcError)
      }
      
      // Fallback: generate unique booking number if RPC failed
      if (!bookingNumber) {
        // Get company info
        const { data: companyData } = await supabase
          .from('companies')
        ...
        // If no initials stored, generate from company name
        ...
        // Find the max sequence from existing bookings to avoid conflicts
        const { data: maxBooking } = await supabase
          .from('bookings')
          .select('booking_number')
          .eq('company_id', user.company_id)
          .like('booking_number', `${initials}-%`)
          .order('created_at', { ascending: false })
          .limit(1)
          .single()
        
        let sequence = (companyData?.booking_sequence || 0)
        
        // Parse existing sequence from last booking if available
        if (maxBooking?.booking_number) {
          const match = maxBooking.booking_number.match(new RegExp(`^${initials}-(\\d+)`))
          if (match) {
        ...
        const suffix = formData.payment_type === 'foc' ? '-FOC' : formData.payment_type === 'insp' ? '-INSP' : ''
        bookingNumber = `${initials}-${sequence.toString().padStart(6, '0')}${suffix}`
      }
      ...
      const { data, error } = await supabase
        ...
        .insert({
          ...
          hotel_id: formData.is_come_direct ? null : hotelId,
          custom_pickup_location: formData.is_come_direct ? null : customPickupLocation,
          room_number: formData.is_come_direct ? null : (formData.room_number || null),
          pickup_time: formData.is_come_direct ? null : (formData.pickup_time || null),
          is_come_direct: formData.is_come_direct,
      ...
      toast.success(`Booking ${bookingNumber} created successfully!`)
  ...
  return (
    <div className="space-y-4 max-w-4xl mx-auto">
      <PageHeader title="New Booking" description="Create a new booking">
          ...
          <div className="lg:col-span-2 space-y-4">
            ...
            <Card>
              ...
              <CardContent className="space-y-3 px-4 pb-4">
                <div className="grid gap-3 sm:grid-cols-2">
                        ...
                        {programs.map((program) => (
                          <SelectItem key={program.id} value={program.id}>
                            <span className="flex items-center gap-2">
                              ...
                              />
                              {(program as any).nickname || program.name}
                            </span>
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  {/* Activity Date with Calendar Picker */}
                  <div className="space-y-1.5">
                      ...
                      <PopoverTrigger asChild>
                        ...
                        >
                          ...
                          {formData.activity_date ? (
                            format(parse(formData.activity_date, 'yyyy-MM-dd', new Date()), 'dd MMMM yyyy')
                          ) : (
                            <span>Pick a date</span>
                          )}
                        </Button>
                      </PopoverTrigger>
                      <PopoverContent className="w-auto p-0 bg-slate-50 dark:bg-slate-900" align="start">
                        <Calendar
                          mode="single"
                          selected={formData.activity_date ? parse(formData.activity_date, 'yyyy-MM-dd', new Date()) : undefined}
                          onSelect={(date) => {
                            if (date) {
            ...
            <Card>
              ...
              <CardContent className="space-y-3 px-4 pb-4">
                ...
                {/* Guest Count - Enhanced styling with dark mode support */}
                <div className="grid grid-cols-3 gap-3">
                  <div className="space-y-1.5">
                    ...
                    <NumberInput
                      ...
                      className="h-16 text-4xl md:text-4xl font-bold text-center bg-blue-500/10 dark:bg-blue-400/20 border-blue-300 dark:border-blue-500/50 focus:border-blue-400 focus:ring-blue-400"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <NumberInput
                      ...
                      className="h-16 text-4xl md:text-4xl font-bold text-center bg-green-500/10 dark:bg-green-400/20 border-green-300 dark:border-green-500/50 focus:border-green-400 focus:ring-green-400"
                    />
                  </div>
                  <div className="space-y-1.5">
                    ...
                    <NumberInput
                      ...
                      onChange={(value) => handleChange('infants', value)}
                      className="h-16 text-4xl md:text-4xl font-bold text-center bg-orange-500/10 dark:bg-orange-400/20 border-orange-300 dark:border-orange-500/50 focus:border-orange-400 focus:ring-orange-400"
                    />
                  </div>
                </div>
              </CardContent>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 px-4 pb-4">
                ...
                <div className="flex items-center space-x-2 p-3 rounded-lg bg-muted/50 border">
                  <Checkbox
                    id="is_come_direct"
                    checked={formData.is_come_direct}
                    onCheckedChange={(checked) => {
                      handleChange('is_come_direct', checked === true)
                      if (checked) {
                        // Clear pickup fields when come direct is selected
                        handleChange('hotel_id', '')
                        handleChange('custom_pickup_location', '')
                        handleChange('room_number', '')
                        handleChange('pickup_time', '')
                        setUseCustomLocation(false)
                      } else {
                        // Restore default pickup time
                        handleChange('pickup_time', '08:00')
                      }
                    }}
                  />
                  <div className="flex items-center gap-2">
                    <MapPin className="w-4 h-4 text-muted-foreground" />
                    <Label htmlFor="is_come_direct" className="text-sm font-medium cursor-pointer">
                      Come Direct / No pickup needed
                    </Label>
                  </div>
                </div>

                {/* Pickup Location Fields - Hidden when Come Direct is selected */}
                {!formData.is_come_direct && (
                  <>
                    <div className="grid gap-3 sm:grid-cols-2">
                      <div className="space-y-1.5">
