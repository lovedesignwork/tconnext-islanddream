...
export default function InvoicesPage() {
  ...
  const fetchInvoices = async () => {
    ...
    const [invoicesRes, companyRes, agentsRes, paymentTypesRes] = await Promise.all([
      ...
      supabase
        ...
        .eq('company_id', user.company_id)
        .eq('status', 'active')
        .neq('agent_type', 'direct') // Exclude direct booking sources
        .order('name'),
      supabase
  ...
  const generatePdfBase64 = async (invoice: InvoiceWithDetails, type: 'invoice' | 'receipt'): Promise<{ base64: string; filename: string }> => {
    ...
    // Fetch agent pricing for this agent to get proper adult/child prices
    const agentId = invoice.agent_id
    const programIds = [...new Set((freshItems || []).map(item => (item.booking as any)?.program_id).filter(Boolean))]
    
    let agentPricingMap = new Map<string, { adult_agent_price: number | null, child_agent_price: number | null, agent_price: number }>()
    
    if (agentId && programIds.length > 0) {
      const { data: pricingData } = await supabase
        .from('agent_pricing')
        .select('program_id, agent_price, adult_agent_price, child_agent_price')
        .eq('agent_id', agentId)
        .in('program_id', programIds)
      
      if (pricingData) {
        pricingData.forEach(p => {
          agentPricingMap.set(p.program_id, {
            adult_agent_price: p.adult_agent_price,
            child_agent_price: p.child_agent_price,
        ...
        <div class="page invoice-page">
          ...
          <div class="info-section">
            ...
            <div class="info-box invoice-details">
              <div class="info-title">INVOICE</div>
              <div class="info-content">
                <div><span class="label">Invoice #:</span> <strong class="invoice-num">${docNumber}</strong></div>
          ...
          <table class="items-table">
            ...
            <tbody>
              ${pageItems.map(item => {
                const booking = item.booking
                if (!booking) return ''
                const programId = (booking as any).program_id
                const programName = (booking.program as any)?.name || '-'
                const voucherNum = booking.voucher_number || '-'
                
                // Get agent pricing for proper adult/child prices
                const pricing = programId ? agentPricingMap.get(programId) : null
                let adultPrice = 0
                let childPrice = 0
                
                if (pricing) {
                  adultPrice = pricing.adult_agent_price ?? pricing.agent_price ?? 0
                  childPrice = pricing.child_agent_price ?? pricing.agent_price ?? 0
                } else {
                ...
                const bookingDate = new Date(booking.activity_date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })
                
                // Compact guest display
                const guestParts = []
                if (booking.adults > 0) guestParts.push(`${booking.adults} Adult`)
                if (booking.children > 0) guestParts.push(`${booking.children} Child`)
                if (booking.infants > 0) guestParts.push(`${booking.infants} Inf`)
                const guestDisplay = guestParts.join(', ')
                ...
                return `
                  <tr>
                    ...
                    <td class="col-details">
                      ...
                      <div class="tour-info">${programName}${voucherNum !== '-' ? ` • ${voucherNum}` : ''}</div>
                    </td>
                    <td class="col-guest">${guestDisplay}</td>
                    <td class="col-price">${adultPrice.toLocaleString()}</td>
                    <td class="col-total">${item.amount.toLocaleString()}</td>
                  </tr>
                `
              }).join('')}
            </tbody>
      ...
      <head>
        ...
        <style>
          ...
          * {
          ...
          .col-booking { width: 18%; vertical-align: middle; }
          .col-booking .booking-date {
            font-size: 9px;
            color: #666;
            margin-top: 1px;
          }
          .col-details { width: 40%; vertical-align: middle; }
          .col-guest { width: 17%; text-align: center; vertical-align: middle; }
          .col-price { width: 12%; text-align: right; vertical-align: middle; }
          .col-total { width: 13%; text-align: right; font-weight: 600; vertical-align: middle; }
  ...
  return (
    <ProtectedPage pageKey="invoices" pageName="Invoices">
