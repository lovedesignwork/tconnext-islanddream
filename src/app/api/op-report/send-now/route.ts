...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check users table first, then team members
    ...
    const { recipient_emails } = body

    if (!recipient_emails || recipient_emails.length === 0) {
      return NextResponse.json(
        { error: 'Missing recipient emails' },
        { status: 400 }
      )
    }
    ...
    // Get Thailand time and determine report date (tomorrow)
    const now = new Date()
    const thailandOffset = 7 * 60
    const utcOffset = now.getTimezoneOffset()
    const thailandTime = new Date(now.getTime() + (thailandOffset + utcOffset) * 60 * 1000)
    
    // Report is for tomorrow
    const reportDate = addDays(thailandTime, 1)
    const dateStr = format(reportDate, 'yyyy-MM-dd')
    const formattedDate = format(reportDate, 'd MMM yyyy')

    console.log(`[OP Report Send Now] Sending report for date: ${formattedDate}`)
    ...
    for (const recipientEmail of recipient_emails) {
      try {
        ...
        console.log(`[OP Report Send Now] Email sent to ${recipientEmail}`)
        successCount++
      } catch (emailError: any) {
        console.error(`[OP Report Send Now] Failed to send email to ${recipientEmail}:`, emailError)
      }
    }

    return NextResponse.json({
      success: true,
      message: `OP Report sent to ${successCount} recipient(s)`,
      reportDate: formattedDate,
      bookingCount: bookingData.length,
    })
  } catch (error: any) {
    console.error('[OP Report Send Now] Error:', error)
    return NextResponse.json(
      { error: error.message || 'Failed to send OP report' },
      { status: 500 }
    )
  }
}
