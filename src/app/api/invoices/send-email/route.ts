...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get user profile for company_id - check users table first, then team members
    ...
    // If not found in users table, check team members
    if (!companyId) {
    ...
    // Get company logo - check both root level and branding
    const companySettings = company?.settings as any
    const companyLogoUrl = companySettings?.logo_url || companySettings?.branding?.logo_url || null
    console.log(`[Invoice Email] Company: ${company?.name}, Logo URL: ${companyLogoUrl || 'NOT FOUND'}`)
    ...
    if (emailType === 'receipt') {
      ...
      if (invoice.payment_type_id) {
        const { data: paymentType } = await supabaseAdmin
    ...
    if (emailType === 'invoice' && invoice.status === 'draft') {
      await supabaseAdmin
        ...
        .update({
          ...
          last_modified_by_name: userName || authUser.email
        })
        .eq('id', invoiceId)
    }
