...
export default function ProgramBookingPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        if (subdomain) params.append('slug', subdomain)
        params.append('programSlug', slug)
        ...
        if (!data.company || !data.program) {
          setLoading(false)
          return
        }

        setCompany(data.company)
        setProgram(data.program as ProgramWithBookingFields)
        setReferenceHotels(data.referenceHotels || [])
        setCustomLocations(data.customLocations || [])
        setAvailability(data.availability || [])
        setStripeEnabled(data.stripeEnabled || false)
        
        // Booking counts are not included in API for now
        setBookingCounts({})
  ...
  useEffect(() => {
    ...
    // Only show cash option if explicitly enabled (true), not when undefined
    const cashAllowed = bookingConfig.allow_cash_on_tour === true
    
    // If current selection is not available, switch to an available option
    if (formData.payment_method === 'cash' && !cashAllowed && stripeEnabled) {
      setFormData(prev => ({ ...prev, payment_method: 'stripe' }))
    } else if (formData.payment_method === 'stripe' && !stripeEnabled && cashAllowed) {
      setFormData(prev => ({ ...prev, payment_method: 'cash' }))
    } else if (!cashAllowed && stripeEnabled && formData.payment_method === 'cash') {
      setFormData(prev => ({ ...prev, payment_method: 'stripe' }))
    }
  }, [company, formData.payment_method, stripeEnabled])
  ...
  // Check if Cash on Tour is allowed - only if explicitly enabled
  ...
  // Use the stripeEnabled flag from API (already checks keys + settings)
  const isStripeAvailable = stripeEnabled

  // Create payment intent when form is valid and payment method is stripe
  const createPaymentIntent = useCallback(async () => {
    // For single-tenant mode, subdomain can be null - that's OK, we use company.slug instead
    if (!company || !program || !formData.activity_date || !formData.customer_name || !formData.customer_whatsapp) {
      return
    }
    ...
    try {
      const response = await fetch('/api/stripe/create-payment-intent', {
        ...
        body: JSON.stringify({
          companyId: company.id, // Use company.id for reliable lookup
          companySlug: subdomain || company.slug, // Keep for metadata
  ...
  }, [company, program, formData, referenceHotels, subdomain])

  // Auto-create payment intent when form is valid and Stripe is selected
  useEffect(() => {
    // Only trigger if:
    // 1. Stripe payment method is selected
    // 2. Form is valid
    // 3. We don't already have a clientSecret
    // 4. Stripe is available
    const isFormReady = formData.activity_date && 
  ...
  }, [formData.payment_method, formData.activity_date, formData.customer_name, 
      formData.customer_email, formData.customer_whatsapp, formData.pickup_type,
      formData.hotel_id, formData.custom_location, clientSecret, stripeEnabled, 
      submitting, createPaymentIntent])
  ...
  const companyLogoUrl = (companySettings as any).logo_url || brandingSettings.logo_url || null
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <main className="max-w-7xl mx-auto p-4 md:p-8">
        ...
        <div className="lg:grid lg:grid-cols-2 lg:gap-8">
          ...
          <div className="lg:sticky lg:top-24 lg:self-start mb-6 lg:mb-0">
            ...
            {/* Brochure & Itinerary Section */}
            <div className="space-y-3">
              {/* Download Brochure Button */}
              {program.brochure_images && program.brochure_images.length > 0 && (
                <Button 
                  variant="outline" 
                  className="w-full text-lg py-7 bg-neutral-500 hover:bg-neutral-600 border-neutral-500 text-white"
              ...
              {/* Itinerary Accordion */}
              {program.itinerary_html && (
                <Accordion type="single" collapsible className="w-full">
                  <AccordionItem value="itinerary" className="border rounded-lg bg-white">
                    <AccordionTrigger className="px-4 py-3 hover:no-underline hover:bg-slate-50 rounded-lg text-slate-700">
                      <span className="inline-flex items-center gap-2 text-left">
                        <FileText className="w-4 h-4" style={{ color: programColor }} />
                        <span className="font-medium text-slate-700 py-0.5">{(program as any).itinerary_title || 'Itinerary'}</span>
                      </span>
                    </AccordionTrigger>
                    <AccordionContent className="px-4 pb-4">
                      <div 
                        className="prose prose-sm max-w-none text-slate-600"
                        dangerouslySetInnerHTML={{ __html: program.itinerary_html }}
                      />
                    </AccordionContent>
                  </AccordionItem>
                </Accordion>
          ...
          <Card>
            <CardHeader className="pb-3">
              <CardTitle className="flex items-center gap-2 text-lg">
                ...
                Do you need a Pick-up & Drop-off transfer?
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {/* Transfer Options - Radio Group */}
              <RadioGroup
                value={formData.pickup_type === 'come_direct' ? 'come_direct' : 'need_pickup'}
                onValueChange={(value) => {
                  if (value === 'come_direct') {
              ...
              >
                ...
                <div 
                  ...
                  onClick={() => handleChange('pickup_type', 'hotel')}
                >
                  <RadioGroupItem value="need_pickup" id="need-pickup" className="pointer-events-none" />
                ...
                {/* No, I will come by myself option */}
                <div 
                  className={`flex items-center space-x-3 p-4 rounded-lg border-2 transition-colors cursor-pointer ${
                    formData.pickup_type === 'come_direct' 
                  ...
                  onClick={() => handleChange('pickup_type', 'come_direct')}
                >
                  <RadioGroupItem value="come_direct" id="come-direct" className="pointer-events-none" />
                  <div className="flex-1">
                    <span className="font-medium">No, I will come by myself to the meeting point</span>
                  </div>
                </div>
              </RadioGroup>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
                ...
                >
                  ...
                  {isStripeAvailable ? (
                    ...
                    <div className="flex items-start space-x-3 p-4 rounded-lg border-2 border-slate-100 bg-slate-50 opacity-60">
                      ...
                      <div className="flex-1">
                        <span className="font-medium text-slate-400">Pay with Credit/Debit Card</span>
                        <p className="text-sm text-slate-400 mt-1">
                          Card payment is not yet available. Please use Cash on Tour.
                        </p>
                      </div>
                    </div>
                  ) : null}
              ...
              {/* Stripe Payment - Inline credit card form */}
              {formData.payment_method === 'stripe' && isStripeAvailable && (
                <>
                  {!isFormValid ? (
                    <p className="text-sm text-center text-slate-500">
                      Please fill in all required fields to see the payment form
                    </p>
                  ) : !clientSecret ? (
                    <div className="space-y-4">
                      <div className="flex items-center justify-center py-6">
                        <Spinner size="md" className="mr-2" />
                        <span className="text-slate-600">Loading secure payment form...</span>
                      </div>
                    </div>
                  ) : stripePromise ? (
                    <Elements 
                    ...
                    <div className="flex items-center justify-center py-4">
                      <Spinner size="md" className="mr-2" />
                      <span className="text-slate-600">Loading payment form...</span>
                    </div>
                  )}
      ...
      {program.brochure_images && program.brochure_images.length > 0 && (
        <ImageLightbox
          images={program.brochure_images}
          isOpen={lightboxOpen}
...
function CheckoutForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Use redirect: 'if_required' to handle payment verification ourselves
    ...
    // Verify payment was actually completed before showing success
    if (paymentIntent) {
      if (paymentIntent.status === 'succeeded') {
        // Payment successful - call confirm-payment API to create booking
        try {
          const confirmResponse = await fetch('/api/booking/confirm-payment', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
          ...
          if (!confirmResponse.ok) {
            console.error('Booking confirmation failed:', confirmData.error)
            // Even if booking creation fails here, still redirect - webhook may handle it
          } else {
            console.log('Booking created:', confirmData.booking_number)
          }
        } catch (confirmError) {
          console.error('Error calling confirm-payment:', confirmError)
          // Continue to success page - webhook may handle booking creation
        }
        
        // Redirect to success page
        window.location.href = `${window.location.origin}/booking/success?payment=stripe`
      } else if (paymentIntent.status === 'processing') {
        // Payment is being processed (rare for card payments)
        setError('Payment is being processed. Please wait...')
        setProcessing(false)
      } else {
        // Payment not complete (cancelled, failed, or requires further action)
        setError('Payment was not completed. Please try again.')
        setProcessing(false)
      }
    } else {
      // No payment intent returned - redirect may have happened, or something went wrong
      setError('Unable to verify payment. Please check your booking status.')
      setProcessing(false)
    }
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div className="flex items-center justify-center gap-2 p-3 bg-green-50 border border-green-200 rounded-lg">
        ...
        <span className="text-sm font-medium text-green-700">Secure Payment - 256-bit SSL Encryption</span>
      </div>

      {/* Credit Card Form - Only show card payment (no PromptPay or other methods) */}
